...
class DataLoaderLite:
  def __init__(self, B, T, process_rank, num_processes):
    ...
    self.current_position = self.B * self.T * self.process_rank # to ensure each process is striding instead of going over the same data
...
if ddp:
  ...
  ddp_rank = int(os.environ['RANK']) # all the gpu's are the same, but they have diff ranks -- so we can coordinate dtaa 
  ddp_local_rank = int(os.environ['LOCAL_RANK']) 
  ddp_world_size = int(os.environ['WORLD_SIZE']) # total number of processes
  device = f'cuda:{ddp_local_rank}'
  torch.cuda.set_device(device)
  master_process = ddp_rank == 0 # this process needs to be logging, checkpointing -- master gpu that has some extra work to do
else:
  ddp_rank = 0
  ddp_local_rank = 0
  ddp_world_size = 1
...
B = 8 # micro_batch
T = 1024 # seq_length
assert total_batch_size % (B * T * ddp_world_size) == 0, "the total_batch_size is not divisible by B * T * ddp_world_size"
grad_accum_steps = total_batch_size // (B * T * ddp_world_size) # now we have that many more of them due to diff devices
if master_process:
